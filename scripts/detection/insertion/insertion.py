...
# This constant represents the Keccak-256 hash of the "Transfer" event signature:
# Transfer(address, address, uint256)
# Purpose:
# - Used in Ethereum logs to identify "Transfer" events emitted by ERC-20 tokens.
# - Helps filter and detect token transfers efficiently by providing a unique topic identifier.
TRANSFER = "0xddf252ad1be2c89b69c2b068fc378daa952ba7f163c4a11628f55a4df523b3ef"  # ERC-20 "Transfer" event hash

# Topic hash for the TokenPurchase event in Uniswap V1
TOKEN_PURCHASE = "0xcd60aa75dea3072fbc07ae6d7d856b5dc5f4eee88854f5b4abf7b680ef8bc50f"  # Uniswap V1 "TokenPurchase"

# Topic Hash for the ETHPurchase event in Uniswap V1
...
def analyze_block(block_number):
    ...
    try:
        events += w3.eth.filter(
            {"fromBlock": block_number, "toBlock": block_number, "topics": [TRANSFER]}
        ).get_all_entries()
        ...
        if (
            ...
            if event["data"].replace("0x", "") and len(event["topics"]) == 3:
                ...
                if _value > 0 and _from != _to:
                    if (
                        event["address"] + _from in transfer_to
                        and transfer_to[event["address"] + _from]["transactionIndex"]
                        + 1
                        ...
                        if delta <= TOKEN_AMOUNT_DELTA:
                            ...
                            for asset_transfer in asset_transfers[event["address"]]:
                                if (
                                    transfer_to[event["address"] + _from][
                                        "transactionIndex"
                                    ]
                                    < asset_transfer["transactionIndex"]
                                    and event["transactionIndex"]
                                    > asset_transfer["transactionIndex"]
                                    and asset_transfer["transactionHash"].hex()
                                    not in attackers
                                ):
                            ...
                            if event_w:
                                ...
                                if (
                                    ...
                                    if (
                                        ...
                                        if (
                                            tx1["to"] == whale_tx["to"] == tx2["to"]
                                            and tx1["from"] != tx2["from"]
                                        ):
                                            continue
                                        ...
                                        if not exchange_name:
                                            try:
                                                ...
                                                exchange_name = (
                                                    exchange_contract.functions.name().call()
                                                )
                                        ...
                                        if not exchange_name:
                                            try:
                                                ...
                                                exchange_name = (
                                                    exchange_contract.functions.name()
                                                    .call()
                                                    .decode("utf-8")
                                                    .replace("\u0000", "")
                                                )
                                        ...
                                        if not exchange_name:
                                            try:
                                                ...
                                                exchange_name = (
                                                    exchange_contract.functions.converterType()
                                                    .call()
                                                    .capitalize()
                                                )
                                        ...
                                        if not exchange_name:
                                            try:
                                                ...
                                                exchange_name = response["result"][0][
                                                    "ContractName"
                                                ]
                                                if exchange_name.startswith("Bancor"):
                                                    exchange_name = "Bancor"
                                        ...
                                        tx1_event, tx2_event, whale_event = (
                                            None,
                                            None,
                                            None,
                                        )
                                        for transfer_event in events:
                                            if (
                                                not tx1_event
                                                and transfer_event["transactionHash"]
                                                == tx1["hash"]
                                                and transfer_event["address"]
                                        ...
                                        if tx1_event and tx2_event and whale_event:
                                            eth_spent = int(
                                                tx1_event["data"].replace("0x", "")[
                                                    0:64
                                                ],
                                                16,
                                            )
                                            eth_received = int(
                                                tx2_event["data"].replace("0x", "")[
                                                    0:64
                                                ],
                                                16,
                                            )
                                            eth_whale = int(
                                                whale_event["data"].replace("0x", "")[
                                                    0:64
                                                ],
                                                16,
                                            )
                                            ...
                                            for exchange_event in exchange_events:
                                                if (
                                                    exchange_event["transactionHash"]
                                                    == tx1["hash"]
                                                ):
                                            ...
                                            if (
                                                ...
                                                eth_spent = int(
                                                    tx1_event["topics"][2].hex(), 16
                                                )
                                                eth_received = int(
                                                    tx2_event["topics"][3].hex(), 16
                                                )
                                                eth_whale = int(
                                                    tx1_event["topics"][2].hex(), 16
                                                )
                                        ...
                                        if gain != None:
                                            attackers.add(
                                                event_a1["transactionHash"].hex()
                                            )
                                            attackers.add(
                                                event_a2["transactionHash"].hex()
                                            )

                                            print(
                                                "   Index Block Number \t Transaction Hash \t\t\t\t\t\t\t From \t\t\t\t\t\t To \t\t\t\t\t\t Gas Price \t Exchange (Token)"
                                            )
                                            print(
                                                "1. "
                                            ...
                                            finding = {
                                                ...
                                                "first_transaction_eth_amount": str(
                                                    eth_spent
                                                ),
                                                "whale_transaction_eth_amount": str(
                                                    eth_whale
                                                ),
                                                "second_transaction_eth_amount": str(
                                                    eth_received
                                                ),
                                                "first_transaction_token_amount": str(
                                                    _value_a1
                                                ),
                                                "whale_transaction_token_amount": str(
                                                    _value_w
                                                ),
                                                "second_transaction_token_amount": str(
                                                    _value_a2
                                                ),
                                                "interface": interface,
                                            ...
                                            if (
                                                "block_number"
                                                not in collection.index_information()
                                            ):
                                                ...
                                                collection.create_index(
                                                    "first_transaction_eth_amount"
                                                )
                                                collection.create_index(
                                                    "whale_transaction_eth_amount"
                                                )
                                                collection.create_index(
                                                    "second_transaction_eth_amount"
                                                )
                                                collection.create_index(
                                                    "first_transaction_token_amount"
                                                )
                                                collection.create_index(
                                                    "whale_transaction_token_amount"
                                                )
                                                collection.create_index(
                                                    "second_transaction_token_amount"
                                                )
...
def init_process(_prices):
    ...
    mongo_connection = pymongo.MongoClient(
        "mongodb://" + MONGO_HOST + ":" + str(MONGO_PORT), maxPoolSize=None
    )
...
def main():
    if len(sys.argv) != 2:
        print(
            colors.FAIL
            + "Error: Please provide a block range to be analyzed: 'python3 "
            + sys.argv[0]
    ...
    if not ":" in sys.argv[1]:
        print(
            colors.FAIL
            + "Error: Please provide a valid block range: 'python3 "
            + sys.argv[0]
            + " <BLOCK_RANGE_START>:<BLOCK_RANGE_END>'"
            + colors.END
        )
    ...
    if not block_range_start.isnumeric() or not block_range_end.isnumeric():
        print(
            colors.FAIL
            + "Error: Please provide integers as block range: 'python3 "
            + sys.argv[0]
            + " <BLOCK_RANGE_START>:<BLOCK_RANGE_END>'"
            + colors.END
        )
    ...
    print(
        "Running detection of insertion frontrunning attacks with "
